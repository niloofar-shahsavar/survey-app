...
const SurveyResponse = () => {
  ...
  useEffect(() => {
    const fetchSurvey = async () => {
      try {
        setLoading(true);
        const response = await fetch(`http://127.0.0.1:8000/surveys/public/${surveyId}`);
        if (!response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/surveys/public/${surveyId}/submit`, {
        method: 'POST',
        headers: {
  ...
  if (submitted) {
    return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full text-center">
        ...
        <p className="text-gray-600 mb-2">Your response has been recorded successfully.</p>
        <p className="text-gray-600 mb-6">We appreciate your feedback and input.</p>
        <button
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mt-6 text-center text-gray-600">
          <p>Question {Object.values(answers).filter(a => a.trim() !== '').length} of {survey?.questions.length} answered</p>
